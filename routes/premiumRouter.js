...
router.get('/test-register', async (req, res) => {
    ...
    let newUser = await models.User.update({ expiredAt: addTime }, { where: { id: data.id } });
...
router.get('/thanks', async (req, res) => {
    if (req.query.message == 'Successful.') {
        ...
        let data = JSON.parse(Buffer.from(req.query.extraData, 'base64').toString());
        // console.log(data);
        // let data = {
        //   id: user.id,
        //   name: user.name,
        ...
        // Sau đó cập nhật expiredAt trong bảng User, bảng này tổng hợp all thông tin về các expiredAt
        let newUser = await models.User.update({ expiredAt: addTime, role: 'premium' }, { where: { id: data.id } });
